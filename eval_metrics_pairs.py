# -*- coding: utf-8 -*-
"""
Evaluate a fine-tuned Hebrew TrOCR model on a "paired sidecar" test set
(flat directory of {name}.png + {name}.gt.txt, e.g. dataset_matan): CER, WER, BLEU.

Usage:
    python eval_metrics_pairs.py --model models/first_iteration \
        --data-dir /mnt/ssd2/cyttic/datasets/dataset_matan/cleaned_dataset/train_pairs \
        [--beams 1] [--limit 0]
"""
...
def main(args):
    ...
    rows = []
    for png_path in sorted(glob.glob(os.path.join(args.data_dir, "*.png"))):
        gt_path = os.path.splitext(png_path)[0] + ".gt.txt"
        if not os.path.exists(gt_path):
            continue
        with open(gt_path, encoding="utf-8") as f:
            text = f.read().strip()
        rows.append((png_path, text))
    if args.limit:
    ...
    for start in range(0, len(rows), bs):
        ...
        imgs = [Image.open(path).convert("RGB") for path, _ in chunk]
        ...
        for (path, gt), pred in zip(chunk, preds):
            refs.append(gt)
            hyps.append(pred)
